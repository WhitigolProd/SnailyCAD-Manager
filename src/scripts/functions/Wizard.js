...
//             if (wizard.requirements.psql)
//                 resolve(wizard.requirements.psql);
...
//         if (!wizard.requirements.git || !wizard.requirements.node || !wizard.requirements.yarn || !wizard.requirements.psql) {
//             $('.requirements').append(`<p>GIT: <span data-status="${wizard.requirements.git}">${wizard.requirements.git}</span></p>`)
//             $('.requirements').append(`<p>GIT: <span data-status="${wizard.requirements.node}">${wizard.requirements.node}</span></p>`)
//             $('.requirements').append(`<p>GIT: <span data-status="${wizard.requirements.yarn}">${wizard.requirements.yarn}</span></p>`)
//             $('.requirements').append(`<p>GIT: <span data-status="${wizard.requirements.psql}">${wizard.requirements.psql}</span></p>`)
//             $('#rqLoad').hide();
//         } else {
//             $('.requirements').hide();
//         }
...
$(function requirements() {
    ...
    $(() => {
        exec(`where -r C:\\Users\\%username%\\AppData\\Roaming\\ pgadmin4.*`, (err, stdout, stderr) => {
            if (err) {
                console.log(err)
